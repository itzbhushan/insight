...
def handle_suggestions(message):
    ...
    """
    elapsed_time = (datetime.utcnow().timestamp() - message["start_ts_utc"]) * 1000
    seq_id = message["sequence_id"]
    logging.debug(f"Received msg {seq_id}. Took {elapsed_time:.2f} ms.")
    print(f"{seq_id}, {elapsed_time:.2f}", file=out_file, flush=True)


def main():
    parser = ArgumentParser("you-complete-me client")
    parser.add_argument("file", help="Input file with list of questions", type=FileType("r"))
    parser.add_argument("--out-dir", help="Output dir where metrics are stored", default="/tmp")
    parser.add_argument("--rate", help="Messages per second. Defaults to 1 msg/sec. Set to zero to disable rate limiting.", default=1.0, type=float)
    parser.add_argument("--num-messages",help="Number of messags to send. Defaults to reading the entire file.", default=0, type=int)
